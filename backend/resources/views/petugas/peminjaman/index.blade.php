...
{{-- Pesan sukses --}}
@if(session('success')) <div class="mb-4 bg-emerald-50 border border-emerald-200 text-emerald-800 p-4 rounded-lg shadow-sm text-sm">
{{ session('success') }} </div>
@endif

{{-- Pesan error --}}
@if(session('error')) <div class="mb-4 bg-red-50 border border-red-200 text-red-800 p-4 rounded-lg shadow-sm text-sm">
{{ session('error') }} </div>
@endif
            ...
            @forelse($peminjamans as $item)
                <tr class="hover:bg-gray-50 transition align-top">
                    ...
                    <td class="py-3 px-4 border-b text-center">
                        ...
                        @else
                            <span class="text-xs font-semibold text-blue-700 bg-blue-50 px-2.5 py-1 rounded">
                                {{ ucfirst($item->status) }}
                            </span>
                        @endif
                    </td>
